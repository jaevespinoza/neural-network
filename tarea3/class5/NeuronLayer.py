...
class NeuronLayer:
    ...
    def updateWeight(self, inputs, learn):
        for i in range(len(self.neurons)):
            self.neurons[i].adjustWeightWithInput(inputs,learn)
            self.neurons[i].adjustBiasUsingLearningRate(learn)
        if not self.isLast():
            self.nextlayer.updateWeight(self.getOutputs(), learn)
